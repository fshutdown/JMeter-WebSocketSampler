...
public class ServiceSocket {
  ...
  public void onWebSocketBinary(byte[] payload, int offset, int len) {
    ...
    synchronized (parent) {
      log.debug("Received binary message: " + msg);
      String length = " (" + msg.length() + " bytes)";
      logMessage.append(" - Received binary message #").append(messageCounter).append(length);
      addResponseMessage("[Message " + (messageCounter++) + "]\n" + msg + "\n\n");
  ...
  public void onFrame(Frame frame) {
    synchronized (parent) {
      ...
      if (responseExpression == null || responseExpression.matcher(frameTxt).find()) {
        logMessage.append("; matched response pattern").append("\n");
        closeLatch.countDown();
      } else if (!disconnectPattern.isEmpty() && disconnectExpression.matcher(frameTxt).find()) {
        logMessage.append("; matched connection close pattern").append("\n");
        closeLatch.countDown();
        close(StatusCode.NORMAL, "JMeter closed session.");
      } else {
  ...
  public void onClose(int statusCode, String reason) {
    if (statusCode != 1000) {
      ...
      logMessage.append(" - WebSocket conection closed unexpectedly by the server: [")
          .append(statusCode).append("] ").append(reason).append("\n");
      error = statusCode;
    } else {
      logMessage.append(" - WebSocket conection has been successfully closed by the server")
          .append("\n");
  ...
  public boolean awaitClose(int duration, TimeUnit unit) throws InterruptedException {
    logMessage.append(" - Waiting for messages for ").append(duration).append(" ")
  ...
  public boolean awaitOpen(int duration, TimeUnit unit) throws InterruptedException {
    logMessage.append(" - Waiting for the server connection for ").append(duration).append(" ")
        .append(unit.toString()).append("\n");
  ...
  protected void initializePatterns() {
    try {
      logMessage.append(" - Using response message pattern \"").append(responsePattern)
          .append("\"\n");
      responseExpression =
          (responsePattern != null || !responsePattern.isEmpty()) ? Pattern
              .compile(responsePattern) : null;
    } catch (Exception ex) {
      logMessage.append(" - Invalid response message regular expression pattern: ")
    ...
    try {
      ...
      disconnectExpression =
          (disconnectPattern != null || !disconnectPattern.isEmpty()) ? Pattern
              .compile(disconnectPattern) : null;
    } catch (Exception ex) {
      logMessage.append(" - Invalid disconnect regular expression pattern: ")
          .append(ex.getLocalizedMessage()).append("\n");
      log.error("Invalid disconnect regular regular expression pattern: "
  ...
  private void addResponseMessage(String message) {
    ...
    try {
      ...
      logMessage.append(" - Message backlog value not set; using default ")
          .append(WebSocketSampler.MESSAGE_BACKLOG_COUNT).append("\n");
